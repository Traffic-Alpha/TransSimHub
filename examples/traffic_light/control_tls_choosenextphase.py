...
@Description: 控制 Traffic Light Signal, 使用 Choose Next Phase 的动作设计
@LastEditTime: 2023-08-28 16:34:36
'''
import sumolib
...
while conn.simulation.getMinExpectedNumber() > 0:
    # 获得路口信息
    tls_infos = scene_traffic_lights.get_traffic_lights_infos() # 返回路口信息
    logger.info(f'SIM: \n{dict_to_str(tls_infos)}')

    # 控制路口信息
    if tls_infos['htddj_gsndj']['can_perform_action']:
        scene_traffic_lights.control_traffic_lights({'htddj_gsndj':action_index})
        action_index = (action_index+1)%4
        logger.debug(f'SIM: Action Index: {action_index}')
    else: # 此时下发的动作会无效
        scene_traffic_lights.control_traffic_lights({'htddj_gsndj':None})
