"""
Generate random mixed-vehicle background traffic for kowloon_police_uav via
SUMO's randomTrips.py. Outputs env.rou.xml and enables it in map.sumocfg.

Usage:
    conda run -n tshub python random_route.py [--seed N] [--duration SEC]
        [--total-flow F] [--car-ratio R] [--bus-ratio R]
        [--truck-ratio R] [--moto-ratio R] [--fringe-factor F]
        [--weights-prefix PREFIX] [--keep-tmp]
"""
import argparse
import os
import shutil
...
SUMOCFG = SCRIPT_DIR / "map.sumocfg"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate random background traffic for kowloon_police_uav."
    )
    ...
    parser.add_argument(
        "--weights-prefix",
        help=(
            "optional randomTrips.py weights prefix. It loads PREFIX.src.xml "
            "and PREFIX.dst.xml for source/destination edge probabilities"
        ),
    )
    parser.add_argument(
        "--keep-tmp",
...
def run_random_trips(script, cls, args, seed):
    ...
    if args.weights_prefix:
        cmd.extend(["--weights-prefix", args.weights_prefix])
    ...
    if result.returncode != 0:
        ...
        sys.exit(
            f"error: randomTrips.py failed for {cls.name} "
...
def update_sumocfg():
    ...
    if commented in text:
        ...
        if "</input>" not in text:
            sys.exit("error: map.sumocfg has no <input> block; refusing to modify")
        new_text = text.replace("</input>", f"        {active}\n    </input>")
...
def main():
    ...
    if not SUMOCFG.exists():
        sys.exit(f"error: {SUMOCFG} not found")
    ...
    try:
        print(
            f"Generating random trips: duration={args.duration}s, "
            f"total_flow={args.total_flow} veh/s, seed={args.seed}, "
            f"fringe_factor={args.fringe_factor}"
        )
        if args.weights_prefix:
            print(f"Using weights prefix: {args.weights_prefix}")
        ...
        print("\nDone. Run with:")
        print(f"  conda run -n tshub sumo-gui -c {SUMOCFG}")
    finally:
