...
@LastEditTime: 2023-12-21 17:33:28
'''
import numpy as np

from typing import List, Dict, Tuple, Union

def analyze_traffic(state, lane_ids, out_edge_ids):
    ...
    for vehicle_id, vehicle in state.items():
        ...
        if vehicle['vehicle_type'] == 'ego':
            ...
            if road_id not in out_edge_ids: # 离开了 bottleneck 就不再计算
...
def compute_ego_vehicle_features(
    ...
    for ego_id in ego_ids:
        if ego_id in ego_statistics:
            ego_info = ego_statistics.get(ego_id)
            ...
            normalized_speed, normalized_distance = 0, 0
        ego_vectors += [normalized_speed, normalized_distance]
    ...
    for _, lane_info in lane_statistics.items():
        ...
        # all_lane_stats += lane_info[0] + [_i/15 for _i in lane_info[1:4]]
        all_lane_stats.append(lane_info[0])
            
    # Combine the features into a single vector
    feature_vector = ego_vectors + all_lane_stats
