...
@Description: 
@LastEditTime: 2024-07-15 23:15:36
'''
...
def build_offscreen_camera(
    ...
    if _camera_type == OffscreenCameraType.BEV: # 俯视
        camera = OffscreenBEVCamera(camera_np=camera_np, buffer=buffer, tex=tex)
    elif _camera_type == OffscreenCameraType.Front: # 前拍
        camera = OffscreenFrontCamera(camera_np, buffer, tex)
    elif _camera_type == OffscreenCameraType.Side: # 侧拍
        camera = OffscreenSideCamera(camera_np, buffer, tex)
    elif _camera_type == OffscreenCameraType.Chase: # 后拍
        camera = OffscreenChaseCamera(camera_np, buffer, tex)
    else:
