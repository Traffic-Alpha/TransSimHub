...
@Description: TSHub 渲染 3D 的场景
@LastEditTime: 2024-07-08 23:18:55
'''
import re
import math
import numpy as np
...
from ..vis3d_renderer.cameras.off_screen_camera import P3DOffscreenCamera

from .sensors.rgb_sensor import RGBSensor # 定义不同的传感器
...
class TSHubRenderer(BaseRender):
    ...
    def __init__(
        ...
        self._camera_nodes: Dict[str, Union[P3DOffscreenCamera]] = {} # TODO, ，这里面还有一个类型, 就是 P3DShaderStep
    ...
    def sync(self, tshub_obs) -> None:
        ...
        veh_ids = set() # 记录车辆的 id
        
        # 在 render 中更新 vehicle 的信息
        # 有一个 elements, 里面有 vehicle, 初始化之后, 可以进行 create node, update node, render, add sensors
        for veh_id, veh_info in tshub_obs['vehicle'].items():
            ...
            if veh_id in self._vehicle_nodes:
                ...
                self._vehicle_elements[veh_id] = Vehicle3DElement(
                    ...
                    glb_model=self.current_file_path("../_assets_3d/vehicles/simple_car.glb")
                ) # 创建车辆 elements
    ...
    def camera_for_id(self, camera_id: str) -> Union[P3DOffscreenCamera]: # P3DShaderStep
        """Get a camera by its id. 根据 camera id 找到对应的 camera, 进行更新.
        """
