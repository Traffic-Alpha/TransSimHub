...
class _ShowBaseInstance(ShowBase):
    ...
    def __new__(cls):
        ...
        if "__it__" not in cls.__dict__:
            ...
            aux_displays = [
                ...
                "pandagles", "pandagles2", "p3headlessgl"
            ]
            ...
            configs = {
                ...
                "model-cache-dir": "/tmp/tshub_panda3d_cache", # 避免使用只读 HOME cache, 也避免读到旧资产缓存。
                "framebuffer-multisample": "1", # 启用帧缓冲区多重采样，这是抗锯齿技术的一种，可以提高渲染图形的质量。
                "multisamples": "4", # 设置多重采样的数量。多数离屏 OpenGL 后端稳定支持 4x MSAA。
                ...
                # 线程模型: 使用默认的单线程 (空). 本渲染器是「渲染一帧 -> 立刻同步读回」的
                # 离屏模式, 分离的 Cull/Draw 线程会让 renderFrame 异步 (绘制/读回延后一帧),
                # 导致 sensor 读到上一帧的残留图 (差一帧). 单线程下 renderFrame 阻塞到读回完成,
                # 一次渲染即当前帧, 正确且配合「先查后渲」拿到 ~2x 提速.
                "threading-model": "" # 单线程 (原为 "Cull/Draw")
            }
    ...
    def init(self) -> None:
        ...
        try:
            ...
            self._assert_gpu_backend_available()
            # 保存 pipeline: 之后 SceneLoader 会按天空类型给它设 env_map (IBL 环境光照)
            ...
            self.setBackgroundColor(0.62, 0.74, 0.86, 1) # sky fallback when skybox is unavailable
            self.setFrameRateMeter(True) # 是否显示 FPS
            logger.info("SIM: 初始化 ShowBase 实例")
        except Exception as e:
            if isinstance(e, RuntimeError) and str(e).startswith("SIM:"):
                raise
            raise RuntimeError(
                "SIM: failed to initialize Panda3D GPU/OpenGL renderer. "
                "tshub3d Panda rendering requires GPU/OpenGL support; please run with "
                "a working OpenGL display or GPU-backed offscreen context."
            ) from e

    def _assert_gpu_backend_available(self) -> None:
        """TinyDisplay is not a supported backend for tshub3d visual rendering."""
        requested = self.__class__._rendering_backend
        pipe_type = str(self.pipe.get_type()) if self.pipe is not None else "unknown"
        if "tiny" in pipe_type.lower():
            raise RuntimeError(
                f"SIM: Panda3D is using '{pipe_type}' (requested={requested}). "
                "tshub3d Panda rendering requires GPU/OpenGL support; TinyDisplay is not supported. "
                "Please run on a machine/session with a working OpenGL display or GPU-backed offscreen context."
            )
        logger.info(f"SIM: Panda3D pipe: {pipe_type} (requested={requested}).")
    ...
    def set_rendering_backend(
        ...
        """
        if rendering_backend == "p3tinydisplay":
            raise ValueError(
                "SIM: 'p3tinydisplay' is not supported. tshub3d Panda rendering "
                "requires GPU/OpenGL support."
            )
        if "__it__" not in cls.__dict__:
    ...
    def setup_sim_root(self, simid: str):
        ...
        # 注: 这里原先给 root_np 套了一个 unlit flat shader, 但 SceneLoader.setup_lighting
        # 随后会在同一节点调用 setShaderAuto(), 把它覆盖掉 —— unlit shader 从未生效 (死代码).
        # 实际着色管线 = simplepbr 自动着色 + 环境光/方向光 + glb 材质颜色, 已移除该死代码.
