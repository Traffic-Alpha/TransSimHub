...
class SceneLoader(object):
    ...
    def load_humans(self):
        """加载路边行人 (静态景观人物; humans.glb; 不存在则跳过)."""
        path = self.scenario_glb_dir / SceneLoader.HUMANS_FILENAME
        if not path.exists():
            return None
        logger.info(f"SIM: 加载路边行人, {path}.")
        model = self._showbase_instance.loader.loadModel(path, noCache=True)
        humans_np = self._root_np.attachNewNode("humans")
        model.reparent_to(humans_np)
        humans_np.hide(CamMask.AllOn)
        humans_np.show(CamMask.MapMask)  # 环境几何
        tag_seg(humans_np, 'person')  # 语义分割: 行人
        logger.info(f"SIM: 路边行人加载成功.")
        return humans_np
