...
用法 (在 tshub 仓库根目录):
    /home/wmn/blender/blender --background --python \
        ...
        scene.json out_dir [--mode lane|env]

mode=lane: 仅路面/路缘/车道线/地面 (聚焦车辆展示);
mode=env : 额外挤出建筑灰模 (需 scene_export 时带 --buildings).
...
DEFAULT_ASSET_DIR = Path(__file__).resolve().parents[3] / "_assets_3d" / "environment"
...
ASSET_LIMITS = {"buildings": 8, "trees": 8, "humans": 6}
...
# --------------- 构建 --------------- #
def build(scene_json, out_dir, mode='city', asset_dir=None):
    ...
    # 4. 城市道具: 沿路树木 / 路边行人 / 建筑 (有 OSM 按 footprint, 否则沿街摆放).
    #    位姿由 Stage 1 (scene_scatter, shapely) 计算; 这里只按位姿实例化共享模板.
    scatter = data.get('scatter', {})
    tree_templates = _load_templates(asset_dir, "trees")
    human_templates = _load_templates(asset_dir, "humans")

    vegetation_objs = instance_props(tree_templates, scatter.get('trees', []), "tree")
    human_objs = instance_props(human_templates, scatter.get('humans', []), "human")
    ...
    for objs, fname in ((building_objs, "buildings.glb"),
                        ...
                        (human_objs, "humans.glb")):
        path = os.path.join(out_dir, fname)
        if objs:
            export_group(objs, path)
        elif os.path.exists(path):
            os.remove(path)
    print(f"BUILD_DONE: {out_dir} (mode={mode}, style=city_builder, roads={len(data['roads'])}, "
          ...
          f"humans={len(human_objs)}, assets={asset_dir})")


if __name__ == '__main__':
    scene_json, out_dir, mode, asset_dir = parse_args()
    build(scene_json, out_dir, mode, asset_dir)
