...
from tshub.tshub_env3d.scene.utils.coordinates import BoundingBox, Point

# 地图元素
from .map_elements.base_road_map import RoadMap
from .scene_generation_utils.geometry import buffered_shape
from .map_elements.lane import Lane
from .map_elements.road import Road
from .map_elements.feature import Feature # TODO, 确认一下 Feature 是否有用到
# 地图元素转换为 glb
from .element_to_glb.ground_glb import make_ground_glb
from .element_to_glb.map_glb import make_map_glb
from .element_to_glb.road_glb import make_road_glb
from .element_to_glb.lane_glb import make_line_glb
...
class SumoNet3D():
    ...
    def to_scene_json(self, out_json: str, buildings_poly: str = None,
                      heights_csv: str = None, apron_width: float = 5.0) -> str:
        """Export renderer-ready static scene geometry as JSON.

        This is the preferred Stage 1 path.  Use Blender build_scene.py as Stage
        2 to write map.glb, ground.glb, road_lines.glb, lane_lines.glb and
        optional buildings.glb with a visual style preset.
        """
    ...
    def to_glb(self, glb_dir:str) -> None:
        ...
        """
        logger.warning(
            "SIM: SumoNet3D.to_glb() is the legacy direct-GLB path. "
            "Prefer to_scene_json() + blender/build_scene.py for styled static assets."
        )
    ...
    def _compute_road_polygons(self) -> List[Tuple[Polygon, Dict[str, Any]]]:
        ...
        for edge in self._graph.getEdges():
            # 跳过 internal (junction connection) 边: 交叉口面积由下方 node 多边形统一
            # 覆盖, 不再叠加一条条 connection 车道带 (否则交叉口重叠发黑).
