...
def plot_vehicle(ax, focus_id:str=None, vehicles:Dict[str, Dict[str, float]]=None) -> List:
    """绘制车辆, 返回新增的 artist 列表 (调用方据此在下一帧移除它们)"""
    artists = []
    for veh_id, veh_info in vehicles.items():
        # Create a rectangle with center at origin
        # 注意: facecolor 与 edgecolor 要分开传, 传 color 会覆盖 edgecolor 并触发 UserWarning
        rectangle = patches.Rectangle(
            (-veh_info['length'], -veh_info['width']/2.0),
            veh_info['length'],
            veh_info['width'],
            facecolor='orange' if focus_id == veh_id else 'blue',
            alpha=0.5,
            edgecolor='black',
            linewidth=1.5
        )
