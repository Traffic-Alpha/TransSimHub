...
@LastEditTime: 2024-07-20 14:21:08
'''
import sumolib
...
class BaseTLS(ABC):
    ...
    def __init__(self, ts_id, sumo):
        ...
        self.tls_position = self.sumo.junction.getPosition(self.id) # 路口的 position
        self.tls_shape = self.sumo.junction.getShape(self.id) # 路口的 shape
        self.tls_radius = sumolib.geomhelper.distancePointToPolygon(self.tls_position, self.tls_shape)
        ...
        self.in_roads_heading = sorted(
            [self.sumo.edge.getAngle(_road_id) for _road_id in self.in_roads]
        )
