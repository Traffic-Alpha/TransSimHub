'''
@Author: Karol Stosiek, Michael Behrisch
@Date: 2021-12-01 21:42:46
@Description: 来自 tools/turn-defs/connections.py
@LastEditTime: 2023-08-31 20:01:08
'''
import os
import sys
...
class UniformDestinationWeightCalculator():
    ...
    def calculate_weight(self,
        ...
        logger.debug("SIM: Destination weight for lane %s/%s connected to %s "
                          "destinations: %f" %
...
class Connections:
    ...
    def add(self,
        ...
        if destination in self.connections_map[source][source_lane]:
            logger.warning("SIM: Destination for %s (lane %s) readded: %s"
    ...
    def calculate_destination_weight(self,
        ...
        logger.debug("SIM: Destination weight for connection "
                          "%s (%s) -> %s: %f" %
...
def from_stream(input_connections):
    ...
    logger.info("SIM: Reading connections from input stream")
